...
const ASTORIA_CATALOG: Record<string, {
  description: string;
  stage: PortfolioStage;
  nextAction: string;
}> = {
  BDDB: {
    description: '製造商上傳型錄後，自動尋找海外經銷商並生成開發信的 Catalox 原型。',
    stage: 'active',
    nextAction: '建立 Git repository，補上真實資料的端到端驗證與資料品質基線。',
  },
  Cuckoo: {
    description: '配方驅動、本地優先的餐廳 POS、庫存、KDS 與營運桌面系統。',
    stage: 'active',
    nextAction: '整理本機領先提交，完成正式 release、CI 產物與真實門店驗證。',
  },
  HyphenNetwork: {
    description: '將 AI 與資料工作拆成可驗證 Work Blocks，由異質節點和專門 Agent 執行。',
    stage: 'active',
    nextAction: '補上沙箱、真實簽名、持久化佇列與節點客戶端，並驗證實際需求。',
  },
  MomoContractor: {
    description: 'MOMOTOY 內部 B2B 客戶、授權、報價、訂單與合同管理工具。',
    stage: 'maintenance',
    nextAction: '完成客戶詳情、PDF、郵件、發票與到期提醒，恢復可驗證的迭代節奏。',
  },
  MomoLandingpage: {
    description: 'MOMOTOY 官網、產品展示、詢價流程與後台 CMS。',
    stage: 'active',
    nextAction: '先同步遠端變更，再跑部署前驗證、Lighthouse 與圖片優化。',
  },
  Party: {
    description: '城市原生活動探索、報名、票券、簽到與主辦方營運平台。',
    stage: 'active',
    nextAction: '完成 20-flow 真機驗收、部署網域確認、通知自動化與支付 ADR。',
  },
  Prophet: {
    description: '技能樹、遊戲化與社群治理結合的跨域概念學習平台。',
    stage: 'active',
    nextAction: '完成 Supabase migrations、伺服端批改、Paddle webhook 與題庫 DB seed。',
  },
  QuoteMe: {
    description: '以一句話為核心、採集單生產的台灣文字服飾平台。',
    stage: 'planning',
    nextAction: '先製作首批效果圖、詢價並取得真實付款，不提前擴張平台開發。',
  },
  School: {
    description: '與 Prophet 共用身份與訂閱的創作者課程市集。',
    stage: 'planning',
    nextAction: '初始化 scaffold 與 Git，確認抽成、影片方案、退款與權限邊界。',
  },
  comma: {
    description: 'Research-first、任務狀態機驅動的 AI 工程編排系統。',
    stage: 'active',
    nextAction: '校準 estimation、監控 prompt cache、擴充 reference pool 並驗證本地模型。',
  },
  'computer.com.tw': {
    description: 'Computex 台灣科技公司資料庫、富化管線與 Agent API。',
    stage: 'active',
    nextAction: '解除富化流程的 API key 阻塞，持續提升全量公司的聯絡資料品質。',
  },
  devassets: {
    description: '獨立開發者的 credential 與環境資產管理 CLI、MCP 和本機儀表板。',
    stage: 'active',
    nextAction: '部署 Hyphen landing、自訂網域，並補齊 Smithery 與 MCP 生態推廣。',
  },
  hypermemresearch: {
    description: '以 typed temporal hypergraph 研究 Agent 工作流記憶與檢索。',
    stage: 'active',
    nextAction: '同步 roadmap 與現況，完成 H2 benchmark、統計顯著性與 H3 負向抑制。',
  },
  'meetily-zh': {
    description: 'Meetily 的繁中、本地優先會議轉錄與即時翻譯 fork。',
    stage: 'active',
    nextAction: '改善 diarization 安裝與穩定性，完成 speaker identification 並同步上游。',
  },
  mindset: {
    description: '每個節點都是 Markdown 的 AI-native 本地心智圖桌面產品。',
    stage: 'active',
    nextAction: '完成簽名與正式發行驗證，修正付款授權安全並取得真實用戶回饋。',
  },
  packpacker: {
    description: '從需求或照片生成包裝、紙板家具與展架生產圖的結構智能平台。',
    stage: 'planning',
    nextAction: '先完成模板、DXF 與 Three.js preview，並用實驗驗證核心結構技術。',
  },
  paperclip: {
    description: '合約語意 diff、條款分類、風險分析與 redline 的桌面工作台。',
    stage: 'active',
    nextAction: '完成 Postgres、Redis、Supabase Auth 整合測試並建立遠端備份。',
  },
  salva: {
    description: '面向 Agent 的自架 discovery intelligence runtime。',
    stage: 'active',
    nextAction: '收斂跨語言 entity resolution 與圖搜尋實驗，明確證明核心價值。',
  },
  sideproject: {
    description: 'Indie hacker 靈感捕捉、項目展示與 build-in-public 平台。',
    stage: 'active',
    nextAction: '完成 OAuth、RLS、PWA 外部設定，觀測真實行為數據並更新 README。',
  },
  sparkie: {
    description: '面向長時間 AI coding session 的本地終端、記憶、任務與 secret 工作台。',
    stage: 'active',
    nextAction: '完成 MCP rate limit、Vault audit、public beta DMG、簽名與 landing page。',
  },
  'worlds-of-dao': {
    description: '玩家以天道身份觀察和干預修仙世界的放置沙盒遊戲。',
    stage: 'active',
    nextAction: '完成 iOS 真機、32 格世界 profiling、經濟重平衡與招牌美術資產。',
  },
  _legacy_computertw: {
    description: 'Computex 廠商與聯絡人資料平台的舊版腳本和資料。',
    stage: 'archived',
    nextAction: '遷移仍有價值的資料到 computer.com.tw，之後保持唯讀封存。',
  },
  hermes_workspace: {
    description: 'Sotir Agent Workspace OS 與 BD automation 的混合工作目錄。',
    stage: 'active',
    nextAction: '拆清產品邊界與 repository，統一版本控制並收斂未完成 phase。',
  },
  OLD: {
    description: 'OpenClaw、BD、交易與營運代理的歷史工作區集合。',
    stage: 'archived',
    nextAction: '維持唯讀歸檔，避免將 credentials、logs 或運行狀態公開推送。',
  },
};
...
function inspectProject(
  ...
  const catalog = ASTORIA_CATALOG[name];
  ...
  const description = catalog?.description ?? extractDescription(name, projectPath, stack);
  const stage = catalog?.stage ?? inferStage(name, projectPath, stack, git);
  ...
  return {
    ...
    nextAction: catalog?.nextAction ?? inferNextAction(stage, git, github, assets),
    evidence,
    git,
    github,
    assets,
